...
public class SImpleTool : Editor
{
    ...
    private static void MoveToZero()
    {
        ...
        for (int i = 0; i < boardItem.cubeItems.Count; i++)
        {
            max_x = max_x < boardItem.cubeItems[i].x ? (int)boardItem.cubeItems[i].x : max_x;
            max_y = max_y < boardItem.cubeItems[i].z ? (int)boardItem.cubeItems[i].z : max_y;

            min_x = min_x > boardItem.cubeItems[i].x ? (int)boardItem.cubeItems[i].x : min_x;
            min_y = min_y > boardItem.cubeItems[i].z ? (int)boardItem.cubeItems[i].z : min_y;
        }
